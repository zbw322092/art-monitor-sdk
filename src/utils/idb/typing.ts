...
export interface IDB {
	 ...
	 */
	transaction(storeNames: string | string[], mode?: 'readonly' | 'readwrite'): ITransaction;
}
...
export interface IUpgradeDB {
	 ...
	 */
	createObjectStore<TValue = any, TKey = any>(name: string, optionalParameters?: IDBObjectStoreParameters): IObjectStore<TValue, TKey>;
...
export interface ITransaction {
	...
	/** The mode for isolating access to data in the object stores that are in the scope
	 * of the transaction. For possible values, see the Constants section below. The default value is readonly.
	 */
	 ...
	 */
	objectStore<TValue = any, TKey = any>(name: string): IObjectStore<TValue, TKey>;
}
...
export interface HasCursor<TValue, TKey> {
	 ...
	 * @param range Optional. A key or IDBKeyRange to be queried. If a single valid key is passed,
	 * this will default to a range containing only that key. If nothing is passed, this will default to a key range that selects all the records in this object store.
	 ...
	 */
	openCursor(range?: IDBKeyRange | IDBValidKey | null, direction?: 'next' | 'nextunique' | 'prev' | 'prevunique'): Promise<ICursor<TValue, TKey>>;
	 ...
	 * @returns A promise that resolves with the cursor once it has been opened.
	 */
	openKeyCursor(range?: IDBKeyRange | IDBValidKey | null, direction?: 'next' | 'nextunique' | 'prev' | 'prevunique'): Promise<ICursor<TValue, TKey>>;
...
export interface IObjectStore<TValue, TKey> extends HasCursor<TValue, TKey> {
	...
	/** Returns a Promise of an IDBRequest object that (in a separate thread) resolves with the objects
	 * in the object store matching the specified parameter or all objects in the store if no parameters are given.
	 * @param query Optional. A key or IDBKeyRange to be queried. If nothing is passed, this will default
	 * to a key range that selects all the records in this object store.
	 * @param count Optional. Specifies the number of values to return if more than one is found. If it is
	 * lower than 0 or greater than 232-1 a TypeError exception will be thrown.
	...
	/** Returns a Promise of an IDBRequest object that (in a separate thread) resolves with record keys for
	 * all the objects matching the specified parameter or all record keys in the store if no parameters are given.
	 * @param query Optional. A key or IDBKeyRange to be queried. If nothing is passed, this will default to a key
	 * range that selects all the records in this object store.
	 * @param count Optional. Specifies the number of values to return if more than one is found. If it is lower
	 * than 0 or greater than 232-1 a TypeError exception will be thrown.
	...
	createIndex<TKey = any>(name: string, keyPath: string | string[], optionalParameters?: IDBIndexParameters): IIndex<TValue, TKey>;
	 ...
	 */
	// tslint:disable-next-line no-shadowed-variable
	index<TKey = any>(name: string): IIndex<TValue, TKey>;
}

/** Wrapper of IDBIndex that presents the asynchronous operations as Promises. */
export interface IIndex<TValue, TKey> extends HasCursor<TValue, TKey> {
	...
	/** Returns a Promise of an IDBRequest object that (in a separate thread) resolves with the objects in the object store
	 * matching the specified parameter or all objects in the store if no parameters are given.
	 * @param query Optional. A key or IDBKeyRange to be queried. If nothing is passed, this will default to a key range
	 * that selects all the records in this object store.
	 * @param count Optional. Specifies the number of values to return if more than one is found. If it is lower than 0 or greater
	 * than 232-1 a TypeError exception will be thrown.
	 * @returns A promise that resolves with the items when the underlying getAll IDBRequest is successful.
	 */
	getAll(query?: IDBKeyRange | IDBValidKey, count?: number): Promise<TValue[]>;

	/** Returns a Promise of an IDBRequest object that (in a separate thread) resolves with record keys for all the objects matching the specified parameter or all record keys
	 * in the store if no parameters are given.
	 * @param query Optional. A key or IDBKeyRange to be queried. If nothing is passed, this will default to a key range
	 * that selects all the records in this object store.
	 * @param count Optional. Specifies the number of values to return if more than one is found.
	 * If it is lower than 0 or greater than 232-1 a TypeError exception will be thrown.
	 * @returns A promise that resolves with the record keys when the underlying getAllKeys
	 * IDBRequest is successful.
	 */
	getAllKeys(query?: IDBKeyRange, count?: number): Promise<TKey[]>;
}

/** Wrapper of IDBCursor that presents the asynchronous operations as Promises. */
export interface ICursor<TValue, TKey> {
	/** The key for the record at the cursor's position. If the cursor is outside its range, this is set to
	 * undefined. The cursor's key can be any data type.
	 */
	readonly key: IDBKeyRange | IDBValidKey;

	/** The cursor's current effective primary key. If the cursor is currently being iterated or has iterated outside its range,
	...
	/** Returns a Promise of an IDBRequest object that (in a separate thread) deletes the record at the cursor's position,
	 * without changing the cursor's position.
	 * This can be used to delete specific records.
	 * @returns A promise that resolves when the underlying delete IDBRequest is successful.
	 */
	delete(): Promise<void>;

	/** Returns a Promise of an IDBRequest object that (in a separate thread) updates the value
	 ...
	 */
	advance(count: number): Promise<ICursor<TValue, TKey>>;

	/** Advances the cursor to the next position along its direction, to the item whose key matches the optional key parameter.
	 * @param key Optional. The key to position the cursor at. If no key is specified, the cursor
	 ...
	 */
	continue(key?: IDBKeyRange | IDBValidKey): Promise<ICursor<TValue, TKey>>;
	 ...
	 * @returns The cursor after having been continued to the next or specified record.
	 */
	continuePrimaryKey(key?: IDBKeyRange | IDBValidKey, primaryKey?: TKey): Promise<ICursor<TValue, TKey>>;
}
